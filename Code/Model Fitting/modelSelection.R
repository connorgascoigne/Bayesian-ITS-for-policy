...
if(!dir.exists(paths = res.model.selection.dir)) {
  dir.create(path = res.model.selection.dir)
}
...
formula2a <- update(formula0, ~ . + f(space_id, graph = lad.mat, model = 'iid', hyper= hyper.pc))
formula2b <- update(formula0, ~ . + f(space_id, graph = lad.mat, model = 'bym2', hyper= hyper.pc.space, scale.model = TRUE, adjust.for.con.comp = TRUE))

### base + time + space ----

formula3aa <- update(formula1a, ~ . + f(space_id, graph = lad.mat, model = 'iid', hyper= hyper.pc))
formula3ab <- update(formula1a, ~ . + f(space_id, graph = lad.mat, model = 'bym2', hyper= hyper.pc.space, scale.model = TRUE, adjust.for.con.comp = TRUE))
formula3ba <- update(formula1b, ~ . + f(space_id, graph = lad.mat, model = 'iid', hyper= hyper.pc))
formula3bb <- update(formula1b, ~ . + f(space_id, graph = lad.mat, model = 'bym2', hyper= hyper.pc.space, scale.model = TRUE, adjust.for.con.comp = TRUE))
...
results.subset <- 
  results %>% 
  dplyr::select(timeModel, spatialModel, DIC, WAIC, time)
